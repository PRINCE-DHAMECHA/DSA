//~ Kruskal's algorithm
//* Greedy to find MST

//* Spanning tree:- Subset of graph that connects all the vertices with minimum edges

//* MST:- Spanning tree having minimum sum of edge weights

//* Sort edges and start making sets greedily

//-----------------------//
//----PRINCE-DHAMECHA----//
//-----------------------//

#include <bits/stdc++.h>
using namespace std;
#define __DONT_RAISE_YOUR_VOICE__IMPROVE_YOUR_ARGUMENT__$ \
    ios::sync_with_stdio(0);                              \
    cin.tie(0)

void makeSet(vector<int> &parent)
{
    for (int i = 0; i < parent.size(); i++)
        parent[i] = i;
}

int find(vector<int> &parent, int x)
{
    if (x != parent[x])
    {
        parent[x] = find(parent, parent[x]);
    }
    return parent[x];
}

void unionSet(vector<int> &parent, vector<int> &rank, int x, int y)
{
    int rootX = find(parent, x);
    int rootY = find(parent, y);
    if (rootX == rootY)
    {
        return;
    }
    if (rank[rootX] > rank[rootY])
    {
        parent[rootY] = rootX;
    }
    else
    {
        parent[rootX] = rootY;
        if (rank[rootX] == rank[rootY])
        {
            rank[rootY]++;
        }
    }
}

bool comp(const tuple<int, int, int> &a, const tuple<int, int, int> &b)
{
    int first = get<2>(a);
    int second = get<2>(b);
    return first < second;
}

void addEdge(vector<vector<pair<int, int>>> &graph, vector<tuple<int, int, int>> &allEdges, int u, int v, int w)
{
    graph[u].push_back(make_pair(v, w));
    graph[v].push_back(make_pair(u, w));
    allEdges.emplace_back(make_tuple(u, v, w));
}

vector<tuple<int, int, int>> kruskal(vector<vector<pair<int, int>>> &graph, vector<tuple<int, int, int>> &allEdges)
{
    vector<int> parent(graph.size());
    vector<int> rank(graph.size());
    vector<tuple<int, int, int>> res;
    makeSet(parent);
    sort(allEdges.begin(), allEdges.end(), comp);
    for (auto i : allEdges)
    {
        int u = get<0>(i);
        int v = get<1>(i);
        int weight = get<2>(i);
        //* Avoid cycle
        if (find(parent, u) != find(parent, v))
        {
            res.push_back({u, v, weight});
            unionSet(parent, rank, u, v);
        }
    }
    return res;
}

int main()
{
    __DONT_RAISE_YOUR_VOICE__IMPROVE_YOUR_ARGUMENT__$;
    int V = 7;
    vector<vector<pair<int, int>>> graph(V, vector<pair<int, int>>(V));
    vector<tuple<int, int, int>> allEdges;

    addEdge(graph, allEdges, 0, 1, 2);
    addEdge(graph, allEdges, 0, 3, 7);
    addEdge(graph, allEdges, 0, 5, 2);
    addEdge(graph, allEdges, 1, 2, 1);
    addEdge(graph, allEdges, 1, 3, 4);
    addEdge(graph, allEdges, 1, 4, 3);
    addEdge(graph, allEdges, 1, 5, 5);
    addEdge(graph, allEdges, 2, 4, 4);
    addEdge(graph, allEdges, 2, 5, 4);
    addEdge(graph, allEdges, 3, 4, 1);
    addEdge(graph, allEdges, 3, 6, 5);
    addEdge(graph, allEdges, 4, 6, 7);

    vector<tuple<int, int, int>> res = kruskal(graph, allEdges);
    cout << "Minimum Spanning Tree: " << endl;
    for (tuple<int, int, int> i : res)
    {
        cout << get<0>(i) << "-" << get<1>(i) << "," << get<2>(i) << endl;
    }
    return 0;
}